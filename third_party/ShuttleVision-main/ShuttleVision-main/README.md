...
## v1 Model Results (Phase 2 complete)

First training run of `yolov8m-pose` on the multi-class pose dataset
(Approach A). Weights: [runs/pose/v1-2/weights/best.pt](runs/pose/v1-1/weights/best.pt).

| Class | Box mAP50 | Box mAP50-95 | Verdict |
|-------|-----------|--------------|---------|
| Player | 0.995 | 0.941 | Excellent — finds players and draws tight boxes |
| badminton_court (Box) | 0.995 | 0.995 | Perfect — court is a giant easy target |
| badminton_court (Pose) | 0.995 | 0.952 | Excellent — homography input is reliable, this unblocks Phase 3 |
| Birdie | 0.458 | 0.172 | Bad — *and* the gap between 0.46 and 0.17 means even when it does detect, the boxes are sloppy |

**Takeaway:** Player and court detection are production-ready and the court keypoints are
strong enough to move into Phase 3 (homography). Birdie (shuttlecock) detection is the
clear bottleneck — next iteration will add 100–200 more shuttlecock-focused annotations,
especially blurry and distant frames, before retraining.
